...
cur_dir=$(pwd)
repo_raw_url="https://raw.githubusercontent.com/jtmyd-top/xrayr-backup/main"
...
install_XrayR() {
    ...
    file="${repo_raw_url}/runtime/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        wget -q -N --no-check-certificate -O /etc/XrayR/config.yml ${repo_raw_url}/defaults/config.yml
        echo -e ""
    ...
    if [[ ! -f /etc/XrayR/dns.json ]]; then
        wget -q -N --no-check-certificate -O /etc/XrayR/dns.json ${repo_raw_url}/defaults/dns.json
    fi
    if [[ ! -f /etc/XrayR/route.json ]]; then
        wget -q -N --no-check-certificate -O /etc/XrayR/route.json ${repo_raw_url}/defaults/route.json
    fi
    if [[ ! -f /etc/XrayR/custom_outbound.json ]]; then
        wget -q -N --no-check-certificate -O /etc/XrayR/custom_outbound.json ${repo_raw_url}/defaults/custom_outbound.json
    fi
    if [[ ! -f /etc/XrayR/custom_inbound.json ]]; then
        wget -q -N --no-check-certificate -O /etc/XrayR/custom_inbound.json ${repo_raw_url}/defaults/custom_inbound.json
    fi
    if [[ ! -f /etc/XrayR/rulelist ]]; then
        wget -q -N --no-check-certificate -O /etc/XrayR/rulelist ${repo_raw_url}/defaults/rulelist
    fi
    curl -o /usr/bin/XrayR -Ls ${repo_raw_url}/runtime/XrayR-manager.sh
